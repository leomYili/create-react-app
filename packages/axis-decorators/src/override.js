...
class SyntaxErrorReporter {
    ...
    get parentNotation() {
        return `${this.parentKlass.constructor.name}#${
            this.parentPropertySignature
        }`;
    }

    get childNotation() {
        return `${this.childKlass.constructor.name}#${
...
function checkDataDescriptors(parent, child, reporter) {
    ...
    if (parentValueType !== childValueType) {
        const isFunctionOverUndefined =
            childValueType === "function" && parentValueType === undefined;
...
function checkDescriptors(parent, child, reporter) {
    ...
    if (parentType !== childType) {
        reporter.error(
            `descriptor types do not match. {parent} is "${parentType}", {child} is "${childType}"`
        );
    }
...
function handleDescriptor(target, key, descriptor) {
    ...
    const reporter = new SyntaxErrorReporter(
        superKlass,
        target,
        superDescriptor,
        descriptor
    ...
    if (superDescriptor === undefined) {
        ...
        const suggestion = suggestedKey
            ...
            : "";
        reporter.error(
            `No descriptor matching {child} was found on the prototype chain.${suggestion}`
        );
    }
